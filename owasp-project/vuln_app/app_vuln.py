...
def search():
    ...
    # ❌ VULNERABLE
    query = f"SELECT id, name, bio FROM users WHERE name LIKE '%{q}%'"
    c.execute(query)
...
def profile():
    if request.method == "POST":
        ...
        conn = sqlite3.connect(DB)
        c = conn.cursor()
        c.execute("UPDATE users SET bio=? WHERE id=1", (new_bio,))
        conn.commit()
        conn.close()

        return "Profile updated (vulnerable)."
